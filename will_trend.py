...
def candles_em_tendencia_forte(historico):
    ...
    # Verifica se os últimos 5 candles estão em tendência de alta ou baixa
    tendencia_alta = all(historico[i] < historico[i + 1] for i in range(-5, -1))
    tendencia_baixa = all(historico[i] > historico[i + 1] for i in range(-5, -1))

    return tendencia_alta or tendencia_baixa
...
def define_direcao(historico):
    ...
    # Começamos definindo a ação na tendência mas depois vamos checar outros dados
    ...
    # Agora decide de muda de tendência para reversão
    if candles_em_variacao(historico):
        if not candles_em_tendencia_forte(historico):
            tendencia = "Reversão"
            if candle_atual_em_alta:
                return "put"
            elif candle_atual_em_baixa:
...
while True:
    try:
        ...
        if comecando_dia:
            ...
            send_slack_notification(f"📈 Iniciando o dia: {fechamento:.5f}, direção: {direcao}, padrão: {tendencia}")
        else:
            server_time = get_server_datetime()
            vela = iq.get_candles(
                ativo,
            ...
            if operacao_aberta:
                ...
                # Processa derrotas
                    ...
                    if qtd_percas_seguidas > max_gales:
                        ...
                        send_slack_notification(f"😥 Perdemos {qtd_percas_seguidas} vezes seguidas, foi mal, encerrando aqui.")
                        exit()
                    else:
